...
const WatchRedirections = () => (
  <section className="bg-[#f1f5f9] py-16 sm:py-24 px-6 overflow-hidden relative border-t border-gray-100">
    ...
    <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8 lg:gap-10 relative z-10">
      ...
      <Link 
        to={`/shop?category=${ProductCategory.MENS_WATCHES}`} 
      ...
      <Link 
        to={`/shop?category=${ProductCategory.WOMENS_WATCHES}`} 
              ...
              {assistantKnowledgeBase.map((item, idx) => (
                <div key={idx} className={`transition-all duration-500 ${activeQuestion === idx ? 'scale-105' : 'scale-100'}`}>
                  <button 
                    ...
                    className={`w-full text-left px-6 py-5 rounded-2xl border transition-all flex items-center justify-between font-bold text-sm ${
                      activeQuestion === idx 
